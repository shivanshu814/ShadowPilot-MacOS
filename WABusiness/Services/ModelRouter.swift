...
enum AnswerMode: String, CaseIterable, Identifiable {
    ...
    // Cloud never goes through a provider chain — Cursor runs it — so its budget
    // is nominal and only kept so the enum stays exhaustive.
    var maxTokens: Int {
        switch self {
        ...
        case .repo:             return 8192
        case .cloud:            return 8192
        }
    }

    // True when the mode is answered by Cursor instead of the provider chain.
...
struct ModelRouter {
    ...
    static func chain(for mode: AnswerMode) -> [RoutedService] {
        ...
        case .coding:
            candidates = [openRouterSmart(), openAI(), openRouterFallback(), groq(), bedrock()]
        case .repo:
            // Long code context + exact line citation — strongest model first, always.
            candidates = [openRouterSmart(), openAI(), openRouterFallback(), bedrock()]
        case .cloud:
            // Cursor runs this one; AppViewModel routes it before a chain is needed.
            candidates = []
        }
    ...
    static func resolveMode(question: String, selected: AnswerMode, previous: AnswerMode?) -> AnswerMode {
        ...
        // A loaded repo wins over generic Code/Review: if the question is about
        // code that actually exists on disk, answer from the real files.
        if RepoIndex.shared.isReady, RepoIndex.shared.looksRepoScoped(question) { return .repo }
...
""" + incrementalRule
    ...
    // Answering with the actual repository open on screen. Everything must be
    // traceable to a real file and a real line — a wrong path or a hallucinated
    // line number is instantly visible to whoever is watching.
    private static let repoInstructions = """
You are answering with the actual repository open in front of you. Below the instructions you get a REPO MAP (every indexed file and what it declares) and RELEVANT CODE (real excerpts, prefixed with their true line numbers).

HARD RULES — these matter more than style:
- Only ever name files, functions and line numbers that appear in the pasted context. Never invent a path, a symbol, or a line range.
- Line numbers must match the gutter exactly. `path/to/file.go:L120-L134` — if you can't see the line, don't cite it.
- The `  123 | ` gutter is display only. NEVER copy it into a code block or diff — diffs must contain the real source lines only.
- If the pasted excerpts don't actually answer the question, say which file you'd open next (pick it from the repo map) and why. That reads as competence; a confident wrong answer does not.

Follow this exact flow, each section SHORT and in my spoken voice:
**Open:** the file(s) I pull up right now — full repo-relative paths, comma-separated. Most relevant first.
**What it does:** 1-2 lines on what that code currently does, citing `path:Lx-Ly`.
**Answer:** the direct answer to what was asked — or, if it's a defect question, the concrete bugs most-severe first, each with its own `path:Lx-Ly`. Nothing generic, nothing textbook.
**Change:** the exact edit as a fenced ```diff block — a few lines of real context, `-` for removed lines, `+` for added. One diff per issue, each preceded by a line reading `// path/to/file.ext  L120-L134`. If nothing needs changing, write "no change needed" and say why.
**Why:** one or two lines — why this fix and what breaks without it.
**What to say:** 2-3 natural spoken lines I can say out loud while I make the edit, so it sounds like I reasoned it out live ("so if you look at line 128, we're returning before the mutex unlocks — that's the leak").

SCOPE — answer exactly what was asked and nothing more:
- "Where is X handled?" → **Open** + **What it does** only. Skip the rest.
- "What's wrong here?" / "fix this" → the full flow.
- A follow-up about one function → just that function, no re-tour of the repo.
""" + incrementalRule

    // Wraps the retrieved code around the interviewer's actual question. `brief`
    // is the last cloud review's findings — prior knowledge, not a substitute for
    // the code: anything cited still has to appear in the excerpts below it.
    static func repoUserPrompt(context: String, question: String, brief: String = "") -> String {
        var p = ""
        let trimmedBrief = brief.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmedBrief.isEmpty {
            let capped = trimmedBrief.count > 7000 ? String(trimmedBrief.prefix(7000)) + "…" : trimmedBrief
            p += """
            === EARLIER DEEP REVIEW OF THIS REPO (background knowledge) ===
            Use it for orientation and to stay consistent with what was already found. It is NOT a source of line numbers — only cite lines you can see in the excerpts below.
    ...
    // One batch of a whole-repo defect sweep. Output is a ranked findings list,
    // not the conversational flow — this runs before/between questions.
    static func bugScanPrompt(batch: String, part: Int, total: Int) -> String {
        """
        You are doing a defect review of part \(part) of \(total) of a real codebase. The source below is prefixed with its true line numbers.

        Report ONLY real, specific defects you can point at in this code — logic errors, nil/index/bounds crashes, race conditions and unsynchronised shared state, resource and goroutine leaks, unhandled errors, off-by-one, incorrect edge-case handling, injection and auth holes, N+1 queries and obvious performance traps. NO style opinions, NO "consider adding tests", NO speculation about code you can't see. If a file is clean, don't mention it.

        For each finding, exactly this shape:

        ### <n>. `path/to/file.ext` L<start>-L<end> — <short title> · <HIGH|MEDIUM|LOW>
        **Bug:** what actually goes wrong, and the input or interleaving that triggers it. One or two lines.
        **Fix:**
        ```diff
        // path/to/file.ext  L<start>-L<end>
        - <real line as it exists>
        + <corrected line>
        ```
        **Say it like this:** one natural spoken line I can say to the interviewer when I point this out.

        Order findings HIGH first. Never copy the `  123 | ` gutter into the diff. If you find nothing real in this batch, reply with exactly: NO FINDINGS.
